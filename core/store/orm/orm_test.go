...
func TestORM_RemoveUnstartedTransaction(t *testing.T) {
	...
	for _, status := range []models.RunStatus{
		...
		ethTx := cltest.MustInsertUnconfirmedEthTxWithBroadcastAttempt(t, storeInstance, 1, key.Address.Address())
	...
	assert.Len(t, runRequests, 1, "expected only RunRequest to be left in the db")

	ethTxes := []models.EthTx{}
	assert.NoError(t, storeInstance.DB.Find(&ethTxes).Error)
	assert.Len(t, ethTxes, 1, "expected only EthTx to be left in the db")

	ethTxAttempts := []models.EthTxAttempt{}
	assert.NoError(t, storeInstance.DB.Find(&ethTxAttempts).Error)
	assert.Len(t, ethTxAttempts, 1, "expected only EthTxAttempt to be left in the db")
}
